...
> **状态**：实施中；State 订阅与全局自动天色同步已明确延期
> **日期**：2026-09-12
> **授权范围**：继续实现 The World；跳过延期的 State 订阅，不将它作为日历、导入、资源诊断等工作的前置。以下早期讨论保留为背景，当前进度以代码与文末实施记录为准。
...
Website 的成熟经验主要体现在背景图与表面层的平滑过渡、透明度算法和 `color-mix` 组合；The World 现已增加独立的 `sky-gradient.ts`，按世界时间在夜间、曙暮光和白天色阶之间使用 smoothstep 插值。该模块只计算调色板与 CSS 渐变，不读取系统时间，也不绑定作者界面。宿主或作者 Renderer 可根据 `WorldTimeComponent` 调用它；背景图存在时，天色应作为低不透明度叠层，避免覆盖资源本身。

## 本轮实施记录：跳过订阅，推进日历检查

- State 订阅与全局自动天色同步延期，正式追踪位置为 LoomStudio 主应用的 `docs/workbench/issues/state-subscription-for-extensions.md`。不再以这项能力阻塞整个扩展。
- 调试台逐一显示带世界时间的实体，按 `calendarId` 精确匹配历法；缺失和重复历法显式诊断。
- 支持作者自定义月名、月份长度、月份浏览和当前日期标记。浏览月份不写 State；没有星期锚点时不推断星期，历法推进与节日规则仍未实现。
- 定向验证：`node --test scripts/chronology.test.js`；人工视觉验收未执行。
- 其余待实现内容继续保留：正式导入预览/提交界面、资源引用闭环、音频的实际使用入口等。本轮没有宣称这些已经完成。
